function loadSvg(slug: string): string {
  try {
    const icon = require('thesvg/' + slug);
    const svg = (icon.variants?.mono || icon.svg || '') as string;
    if (svg) return svg;
  } catch {
    // fallback
  }
...
export interface ChatMessage {
  ...
  timestamp?: string;
  tokens?: number;
  tool_calls?: Array<{ name: string; arguments?: Record<string, any>; content?: string }>;
  is_thinking?: boolean;
...
export interface AgentSession {
  ...
  totalTokens: number;
  projectPath: string;
  source: 'detected' | 'imported' | 'manual';
...
export type Provider = 'Anthropic' | 'OpenAI' | 'Google' | 'GitHub' | 'Unknown';

export interface AgentInfo {
  name: AgentName;
  provider: Provider;
  icon: string;
  dataPaths: { label: string; path: string }[];
  processNames: string[];
}
...
export const KNOWN_AGENTS: AgentInfo[] = [
  ...
  },
  {
    ...
    icon: SVG_QODER,
    dataPaths: [
      { label: 'Qoder CLI data', path: '.qoder' },
    ],
    processNames: ['qoder', 'qoder.exe', 'qodercli', 'qodercli.exe'],
  },
  {
    name: 'VS Code',
    provider: 'GitHub',
